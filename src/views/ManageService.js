...
import ServiceCrudModal from '../components/modals/ServiceCrudModal'
...
const ManageService = () => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axiosInstance.delete(`/api/service/${deleteId}`)
  ...
  const fields = [
    ...
    { name: 'file_img_url2', label: 'Gambar 2', type: 'file', accept: 'image/*', required: true },
    { name: 'file_img_url3', label: 'Gambar 3', type: 'file', accept: 'image/*', required: true },
  ]
  ...
  return (
    <>
      ...
      <CModal visible={deleteModalVisible} onClose={() => setDeleteModalVisible(false)}>
        <CModalBody>Apakah Anda yakin ingin menghapus service ini?</CModalBody>
        <CModalFooter>
